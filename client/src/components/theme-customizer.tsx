...
// ThemeCustomizer: effects-only applier (no UI). Keep in Layout so theme is always applied.
...
// ThemeCustomizerContent: full settings UI used in the Settings page
export function ThemeCustomizerContent() {
  ...
  const applyThemeColors = useCallback((cfg: ThemeConfig, isDark: boolean) => {
    const root = document.documentElement
    ...
    root.dir = cfg.layout
    document.body.dir = cfg.layout
    root.style.setProperty("--radius", `${cfg.card.borderRadius}px`)
    root.style.setProperty("--card-border-width", `${cfg.card.borderWidth}px`)
    root.style.setProperty("--card-padding", `${cfg.card.padding}px`)
    root.style.setProperty("--font-size-base", `${cfg.typography.fontSize}px`)
    root.style.setProperty("--font-weight-base", cfg.typography.fontWeight)
    root.style.setProperty("--line-height-base", cfg.typography.lineHeight.toString())
    root.style.setProperty("--button-radius", `${cfg.button.borderRadius}px`)
    root.style.setProperty("--button-font-size", `${cfg.button.fontSize}px`)
    root.style.setProperty("--button-padding", cfg.button.padding)
    let style = document.getElementById("theme-customizer-styles")
    if (!style) { style = document.createElement("style"); style.id = "theme-customizer-styles"; document.head.appendChild(style) }
    style.textContent = `
      .sidebar-menu-text { color: hsl(${colors.menuText || colors.foreground}) !important; }
      .sidebar-menu-icon { color: hsl(${colors.menuIcon || colors.foreground}) !important; }
      .sidebar-menu-item:hover .sidebar-menu-text { color: hsl(${colors.menuTextHover || colors.foreground}) !important; }
      .sidebar-menu-item:hover .sidebar-menu-icon { color: hsl(${colors.menuIconHover || colors.foreground}) !important; }
      .sidebar-section-label { color: hsl(${colors.menuSectionLabel || colors.mutedForeground}) !important; }
      button, .button, [role="button"] { border-radius: ${cfg.button.borderRadius}px !important; font-size: ${cfg.button.fontSize}px !important; padding: ${cfg.button.padding} !important; }
      body { font-size: ${cfg.typography.fontSize}px !important; font-weight: ${cfg.typography.fontWeight} !important; line-height: ${cfg.typography.lineHeight} !important; }
    `
  }, [])
  ...
  useEffect(() => {
    ...
    if (savedConfig) {
      try {
        const parsed = JSON.parse(savedConfig)
        finalConfig = { ...defaultConfig, ...parsed, colors: { light: { ...defaultConfig.colors.light, ...(parsed.colors?.light || {}) }, dark: { ...defaultConfig.colors.dark, ...(parsed.colors?.dark || {}) } } }
      } catch { finalConfig = defaultConfig }
    }
    setConfig(finalConfig)
    applyThemeColors(finalConfig, theme === "dark")
    if (typeof window !== "undefined" && (window as any).setMenuStateFromCustomizer) {
      ;(window as any).setMenuStateFromCustomizer(finalConfig.menuState)
    }
  }, [mounted, theme, applyThemeColors])
  ...
  useEffect(() => {
    if (!mounted) return
    applyThemeColors(config, theme === "dark")
    if (typeof window !== "undefined" && (window as any).setMenuStateFromCustomizer) {
      ;(window as any).setMenuStateFromCustomizer(config.menuState)
    }
  }, [config, mounted, applyThemeColors])
  ...
  useEffect(() => {
    ...
    const root = document.documentElement
    Object.entries(colors).forEach(([key, value]) => {
      if (value && typeof value === "string") root.style.setProperty(`--${key.replace(/([A-Z])/g, "-$1").toLowerCase()}`, value)
    })
  }, [theme, mounted, config.colors])
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <Select value={config.menuState} onValueChange={(value: "full" | "collapsed" | "hidden") => updateConfig("menuState", value)}>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <ColorInput label="Background" path={`colors.${editingDarkMode ? "dark" : "light"}.background`} value={editingDarkMode ? config.colors.dark.background : config.colors.light.background} />
            <ColorInput label="Foreground" path={`colors.${editingDarkMode ? "dark" : "light"}.foreground`} value={editingDarkMode ? config.colors.dark.foreground : config.colors.light.foreground} />
            <ColorInput label="Primary" path={`colors.${editingDarkMode ? "dark" : "light"}.primary`} value={editingDarkMode ? config.colors.dark.primary : config.colors.light.primary} />
            <ColorInput label="Secondary" path={`colors.${editingDarkMode ? "dark" : "light"}.secondary`} value={editingDarkMode ? config.colors.dark.secondary : config.colors.light.secondary} />
            <ColorInput label="Accent" path={`colors.${editingDarkMode ? "dark" : "light"}.accent`} value={editingDarkMode ? config.colors.dark.accent : config.colors.light.accent} />
            <ColorInput label="Border" path={`colors.${editingDarkMode ? "dark" : "light"}.border`} value={editingDarkMode ? config.colors.dark.border : config.colors.light.border} />
            <ColorInput label="Card" path={`colors.${editingDarkMode ? "dark" : "light"}.card`} value={editingDarkMode ? config.colors.dark.card : config.colors.light.card} />
          </div>
          <Label className="text-sm font-medium text-gray-700 dark:text-gray-300">Menu Colors</Label>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <ColorInput label="Menu Text" path={`colors.${editingDarkMode ? "dark" : "light"}.menuText`} value={editingDarkMode ? config.colors.dark.menuText : config.colors.light.menuText} />
            <ColorInput label="Menu Text Hover" path={`colors.${editingDarkMode ? "dark" : "light"}.menuTextHover`} value={editingDarkMode ? config.colors.dark.menuTextHover : config.colors.light.menuTextHover} />
            <ColorInput label="Menu Icon" path={`colors.${editingDarkMode ? "dark" : "light"}.menuIcon`} value={editingDarkMode ? config.colors.dark.menuIcon : config.colors.light.menuIcon} />
            <ColorInput label="Menu Icon Hover" path={`colors.${editingDarkMode ? "dark" : "light"}.menuIconHover`} value={editingDarkMode ? config.colors.dark.menuIconHover : config.colors.light.menuIconHover} />
            <ColorInput label="Section Labels" path={`colors.${editingDarkMode ? "dark" : "light"}.menuSectionLabel`} value={editingDarkMode ? config.colors.dark.menuSectionLabel : config.colors.light.menuSectionLabel} />
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-4 max-w-sm">
            <div>
              ...
              <Slider value={[config.card.borderRadius]} onValueChange={([v]) => updateConfig("card.borderRadius", v)} max={20} min={0} step={1} className="mt-2" />
            </div>
            <div>
              <Label className="text-xs font-medium">Border Width: {config.card.borderWidth}px</Label>
              <Slider value={[config.card.borderWidth]} onValueChange={([v]) => updateConfig("card.borderWidth", v)} max={5} min={0} step={1} className="mt-2" />
            </div>
            <div>
              <Label className="text-xs font-medium">Padding: {config.card.padding}px</Label>
              <Slider value={[config.card.padding]} onValueChange={([v]) => updateConfig("card.padding", v)} max={48} min={8} step={4} className="mt-2" />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-4 max-w-sm">
            <div>
              ...
              <Slider value={[config.button.borderRadius]} onValueChange={([v]) => updateConfig("button.borderRadius", v)} max={20} min={0} step={1} className="mt-2" />
            </div>
            <div>
              <Label className="text-xs font-medium">Font Size: {config.button.fontSize}px</Label>
              <Slider value={[config.button.fontSize]} onValueChange={([v]) => updateConfig("button.fontSize", v)} max={20} min={10} step={1} className="mt-2" />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-4 max-w-sm">
            <div>
              ...
              <Slider value={[config.typography.fontSize]} onValueChange={([v]) => updateConfig("typography.fontSize", v)} max={20} min={10} step={1} className="mt-2" />
            </div>
            <div>
              <Label className="text-xs font-medium">Font Weight</Label>
              <Select value={config.typography.fontWeight} onValueChange={(value) => updateConfig("typography.fontWeight", value)}>
            ...
            <div>
              ...
              <Slider value={[config.typography.lineHeight]} onValueChange={([v]) => updateConfig("typography.lineHeight", v)} max={2} min={1} step={0.1} className="mt-2" />
            </div>
          </div>
        </div>
