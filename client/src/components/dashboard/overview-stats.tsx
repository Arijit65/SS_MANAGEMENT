  ...
  return (
    <div className="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-4">
      {stats.map((stat) => (
        <div
          key={stat.title}
          className="bg-white dark:bg-[#0F0F12] rounded-lg sm:rounded-xl p-3 sm:p-6 border-l-4 border-[#1DA1F2] border border-gray-200 dark:border-[#1F1F23] hover:shadow-md hover:shadow-[#1DA1F2]/10 transition-shadow"
        >
          <div className="flex items-center justify-between space-y-0 pb-2">
            <h3 className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400 truncate">{stat.title}</h3>
            <div className="text-[#1DA1F2] flex-shrink-0">{stat.icon}</div>
          </div>
          <div className="space-y-1">
            ...
            <div className="flex items-center text-xs">
              {stat.changeType === "increase" ? (
                ...
                <TrendingDown className="h-3 w-3 text-[#1DA1F2] mr-1 flex-shrink-0" />
              )}
              <span className={`font-medium ${stat.changeType === "increase" ? "text-green-600" : "text-[#1DA1F2]"}`}>
                {stat.change}
              </span>
